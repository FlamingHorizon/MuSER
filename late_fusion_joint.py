...
train_a, train_l, train_labels = audios[sp[0]], input_ids[sp[0]], stress_labels[sp[0]]
...
# to Tensors
if TRIconfig.mode == 'regression':
    train_labels, test_labels = torch.FloatTensor(train_labels), torch.FloatTensor(test_labels)
else:
    train_labels, test_labels = torch.LongTensor(train_labels), torch.LongTensor(test_labels)

train_a, test_a = torch.FloatTensor(train_a), torch.FloatTensor(test_a)

train_l, test_l, train_token_type_ids, test_token_type_ids = torch.LongTensor(train_l), \
...
model = jointTA(TRIconfig).to('cuda')
...
for ep in range(max_epochs):
    ...
    print("epoch: %d rmse: %f" % (ep + 1, avg_loss ** 0.5))
