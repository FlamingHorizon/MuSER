...
# from captum.attr import LayerConductance, configure_interpretable_embedding_layer, remove_interpretable_embedding_layer
...
class jointTAMulti(nn.Module):
    ...
    def forward2(self,
        ...
        text_repr = self.text_emb(bert_outputs[1])
        final_repr = torch.cat([audio_repr, text_repr], dim=1)
        # final_repr = audio_repr
        # final_repr = text_repr
        final_repr = self.dropout(final_repr)
        # logits = self.classifier(final_repr)
        # print(logits, labels)
        logi = self.classify(final_repr)
        return logi
...
with open('all_data_omg_ready.pkl', 'rb') as dump_file:
    audios_omg, activation_labels_omg, valence_labels_omg, emo_labels_omg, input_ids_omg, token_type_ids_omg, attention_mask_omg = pkl.load(dump_file)
...
# omg to Tensors
train_num_omg = len(audios_omg) - 300
audios_omg_train, audios_omg_dev, activation_labels_omg_train, activation_labels_omg_dev, valence_labels_omg_train,\
 valence_labels_omg_dev, attention_mask_omg_train, attention_mask_omg_dev = audios_omg[:train_num_omg], \
    audios_omg[train_num_omg:], activation_labels_omg[:train_num_omg], activation_labels_omg[train_num_omg:], \
    valence_labels_omg[:train_num_omg], valence_labels_omg[train_num_omg:], attention_mask_omg[:train_num_omg],\
    attention_mask_omg[train_num_omg:]
input_ids_omg_train, input_ids_omg_dev, token_type_ids_omg_train, token_type_ids_omg_dev = input_ids_omg[:train_num_omg], \
    input_ids_omg[train_num_omg:], token_type_ids_omg[:train_num_omg], token_type_ids_omg[train_num_omg:]

dev_num_omg = len(input_ids_omg_dev)

audios_omg_train, activation_labels_omg_train, valence_labels_omg_train, attention_mask_omg_train = torch.FloatTensor(audios_omg_train), \
    torch.FloatTensor(activation_labels_omg_train), torch.FloatTensor(valence_labels_omg_train), \
    torch.FloatTensor(attention_mask_omg_train)

audios_omg_dev, activation_labels_omg_dev, valence_labels_omg_dev, attention_mask_omg_dev = torch.FloatTensor(audios_omg_dev), \
    torch.FloatTensor(activation_labels_omg_dev), torch.FloatTensor(valence_labels_omg_dev), \
    torch.FloatTensor(attention_mask_omg_dev)

input_ids_omg_train, token_type_ids_omg_train = torch.LongTensor(input_ids_omg_train), torch.LongTensor(token_type_ids_omg_train)
input_ids_omg_dev, token_type_ids_omg_dev = torch.LongTensor(input_ids_omg_dev), torch.LongTensor(token_type_ids_omg_dev)
...
for ep in range(max_epochs):
    ...
    while total_samples < n_train:
        ...
        if selected_task == 0:
            selected_id = np.random.permutation(train_num_omg)[:batch_size]
            ans = activation_labels_omg_train[selected_id].to('cuda')
            model.set_mode('activ')
            batch_a = audios_omg_train[selected_id].to('cuda')
            batch_l = input_ids_omg_train[selected_id].to('cuda')

            batch_ty = token_type_ids_omg_train[selected_id].to('cuda')
            batch_am = attention_mask_omg_train[selected_id].to('cuda')

        elif selected_task == 1:
            selected_id = np.random.permutation(train_num_omg)[:batch_size]
            ans = valence_labels_omg_train[selected_id].to('cuda')
    ...
    n_batch = 0
    dev_preds = np.array([])
    while idx < dev_num_omg:
        # run on each task
        dev_batch_a = audios_omg_dev[idx:(idx + dev_batch_size)].to('cuda')
        dev_batch_l = input_ids_omg_dev[idx:(idx + dev_batch_size)].to('cuda')
        dev_batch_ty = token_type_ids_omg_dev[idx:(idx + dev_batch_size)].to('cuda')
        dev_batch_am = attention_mask_omg_dev[idx:(idx + dev_batch_size)].to('cuda')
        dev_ans_activ = activation_labels_omg_dev[idx:(idx + dev_batch_size)].to('cuda')
        dev_ans_valence = valence_labels_omg_dev[idx:(idx + dev_batch_size)].to('cuda')
        model.set_mode('activ')
    ...
    del dev_batch_l, dev_batch_ty, dev_batch_am, dev_ans_activ, dev_ans_valence, dev_batch_a
    torch.cuda.empty_cache()
    ...
    del dev_batch_l, dev_batch_ty, dev_batch_am, dev_ans_stress, dev_batch_a
    torch.cuda.empty_cache()
    ...
    if ep < 5:
        ...
        rate_active = np.mean(moving_metric_activ[ep - 5:ep]) / (dev_loss_activ + 1e-5) * 10
        rate_valence = np.mean(moving_metric_valence[ep - 5:ep]) / (dev_loss_valence + 1e-5) * 10
        rate_stress = dev_acc / (np.mean(moving_metric_stress[ep - 5:ep]) + 1e-5) * 10
