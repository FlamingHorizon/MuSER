...
epoch_to_second = 100
epoch_to_main = 200
model.set_mode('valence')  # first aux task
...
for ep in range(max_epochs):
    ...
    n_batch = 0
    model.train()
    if ep == epoch_to_second:
        print('=========================SWITCHED TO SECOND AUXILIARY TASK================================')
    elif ep == epoch_to_main:
        print('=========================SWITCHED TO MAIN TASK================================')
    ...
    while idx < n_train:
        ...
        if model.mode == 'activ':
            ans = train_activ[idx:(idx + batch_size)].to('cuda')
        elif model.mode == 'valence':
            ans = train_valence[idx:(idx + batch_size)].to('cuda')
        else:
    ...
    if ep % eval_every == 0:
        ...
        while idx < n_test:
            ...
            if model.mode == 'activ':
                test_ans = test_activ[idx:(idx + test_batch_size)].to('cuda')
            elif model.mode == 'valence':
                test_ans = test_valence[idx:(idx + test_batch_size)].to('cuda')
            else:
            ...
            if model.mode == 'activ' or model.mode == 'valence':
